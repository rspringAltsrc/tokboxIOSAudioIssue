...
(function ($) {
	...
	Annotate.prototype = {
		init: function () {
			...
			$('.annotate-container').append(self.$textbox);
		...
		redraw: function () {
			...
			for (var i = 0; i < self.storedElement.length; i++) {
				...
				switch (element.type) {
					case 'rectangle':
						self.drawRectangle(self.baseContext, element.fromx, element.fromy, element.tox, element.toy, element.color);
						break;
					case 'arrow':
		...
		drawRectangle: function (context, x, y, w, h, color) {
			var self = this;
			context.beginPath();
		...
		drawCircle: function (context, x1, y1, x2, y2, color) {
		...
		annotatestart: function (event) {
			...
			if (self.$textbox.is(':visible')) {
				...
				if (text !== '') {
					...
					self.storedElement.push({
						type: 'text',
						text: text,
						fromx: (self.fromxText) * self.compensationWidthRate,
						fromy: (self.fromyText) * self.compensationWidthRate,
						maxwidth: self.tox,
						color: self.options.color
					});
					if (self.storedUndo.length > 0) {
			...
			self.fromxText = self.fromx;
			self.fromyText = self.fromy;
				...
				switch (self.options.type) {
					...
					case 'pen':
						...
						for (var i = 0; i < self.points.length - 1; i++) {
							...
							self.drawPen(self.baseContext, self.fromx, self.fromy, self.tox, self.toy, self.options.color);
						}
						self.points = [];
						break;
					default:
		...
		annotatemove: function (event) {
			...
			switch (self.options.type) {
				...
				case 'text':
					self.clear();
					self.tox = (pageX - offset.left) * self.compensationWidthRate - self.fromxText;
					self.toy = (pageY - offset.top) * self.compensationWidthRate - self.fromyText;
					self.$textbox.css({
						left: self.fromxText + 2,
						top: self.fromyText,
						width: self.tox - 12,
		...
		changeColor: function (newColor, callback) {
			var self = this;
			self.annotatestop();
			self.options.color = newColor;
		} 
	};
	$.fn.annotate = function (options, cmdOption, callback) {
		...
		if (options === 'destroy') {
			...
			if ($annotate) {
				$annotate.changeColor(cmdOption, callback);
			} else {
